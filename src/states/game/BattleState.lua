...
function BattleState:init(player)
    ...
    self.opponent =
        ...
        party = Party {
            pokemon = {
                Pokemon(Pokemon.getRandomDef(), math.random(2, 20))
            }
        }
    }
...
function BattleState:render()
    ...
    if self.renderHealthBars then
        ...
        love.graphics.print(
            "LV " .. tostring(self.playerPokemon.level),
            self.playerHealthBar.x,
            self.playerHealthBar.y - 10
        )
        love.graphics.print(
            "LV " .. tostring(self.opponentPokemon.level),
            self.opponentHealthBar.x,
            self.opponentHealthBar.y + 8
        )
...
function BattleState:triggerStartingDialogue()
    ...
    gStateStack:push(
        BattleMessageState(
            "A wild " .. tostring(self.opponent.party.pokemon[1].name .. " appeared!"),
            -- callback for when the battle message is closed
            function()
                gStateStack:push(
                    BattleMessageState(
                        "Go, " .. tostring(self.player.party.pokemon[1].name .. "!"),
